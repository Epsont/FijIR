...
//var objLink = document.getElementsByClassName("_on53ahr")[0];
...
links.setAttribute("style","display: flex; float: right;");
...
//ДОБАВЛЕНИЕ КНОПКИ ПЕРЕХОДА В ROSREESTR
const rosreestrLink = document.createElement('div');
rosreestrLink.setAttribute("class", "rosreestr-link");
rosreestrLink.setAttribute("style","width:40px;height:40px;border: #00BFFF;overflow: hidden;padding: 4px;margin-left: 2px;margin-right: 4px;border-radius: 4px;box-shadow: 0 1px 3px 0 rgba(38, 38, 38, 0.5);-moz-box-sizing: border-box;box-sizing: border-box;background: #ffffff;color: #262626;;float:left;");
rosreestrLink.setAttribute("onmouseover","this.style.backgroundColor='#F0F8FF';");
rosreestrLink.setAttribute("onmouseout","this.style.backgroundColor='white';");




//ЗАМЕНА КНОПОК
function addButtons(coord = null, syscodeGeo = null, syscodeFirm = null, moveToFiji = null, moveToFijiBySyscode = null, moveToInforussia = null, moveToYouRa = null, moveToRosreestr = null) {
    ...
    if (coord) {
        objTapUser.append(rosreestrLink);
        var objrosreestrLink = document.getElementsByClassName("rosreestr-link")[0];
        rosreestrLink.innerHTML = moveToRosreestr;
        objrosreestrLink.replaceWith(rosreestrLink);
    } else {
        rosreestrLink.remove();
    };
...
function fijIR() {
    ...
    var moveToRosreestr =  `<a href="https://rosreestr-doc.ru/%D0%BA%D0%B0%D0%B4%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B2%D0%B0%D1%8F_%D0%BA%D0%B0%D1%80%D1%82%D0%B0#ct=${coord[1]}&cg=${coord[0]}&zoom=16&layer=dgis" target="_blank" title="Перейти на rosreest-docs"><img width="32" height="32" src="https://rosreestr-doc.ru/favicon-32x32.png"></a>`;
